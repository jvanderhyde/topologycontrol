...
int volume::openFeatureInside(maxqueue& boundary,int* neighbors,pqitem* neighborsQ)
{
	int x,y,z,index,minindex=-1;
	short min=32767;
	pqitem top;
	...
	{
		if ((!carved.getbit(index)) && (known.getbit(index)) && (data[index]<0) && (data[index]<min))
		{
			getVoxelLocFromIndex(index,&top.x,&top.y,&top.z);
			top.priority=-data[index];
			minindex=index;
			min=data[index];
		}
	}
	...
	if (minindex>-1)
	{
		cout << "Min known negative voxel: (" << top.x << ',' << top.y << ',' << top.z << ")\n";
		carveVoxelInside(top.x,top.y,top.z,minindex,boundary,neighbors,neighborsQ);
		return 1;
	}
	else
	{
...
int volume::openFeatureOutside(maxqueue& boundary,int* neighbors,pqitem* neighborsQ)
{
	int x,y,z,index,maxindex=-1;
	short max=-32767;
	pqitem top;
	...
	for (index=0; index<size[2]*size[1]*size[0]; index++)
	{
		if ((!carved.getbit(index)) && (known.getbit(index)) && (data[index]>0) && (data[index]>max))
		{
			getVoxelLocFromIndex(index,&top.x,&top.y,&top.z);
			top.priority=data[index];
			maxindex=index;
	...
	if (maxindex>-1)
	{
		cout << "Max known positive voxel: (" << top.x << ',' << top.y << ',' << top.z << ")\n";
...
void volume::carveSimultaneously(int numFeaturesToOpen)
{
	...
	while (numFeatures<numFeaturesToOpen)
	{
		...
		if (numFeatures<numFeaturesToOpen)
		{
			...
			openedInside=openFeatureInside(innerBoundary,neighbors,neighborsQ);
			openedOutside=openFeatureOutside(outerBoundary,neighbors,neighborsQ);
...
{
    frameNumber++;
    if (frameNumber%10000000!=0) return;
