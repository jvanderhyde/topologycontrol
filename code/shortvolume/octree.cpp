...
//Adds a new voxel to an fully-constructed octree
...
/*void octree::readData_old(istream& in,const int vsize[3],int treeHeight)
{
    //construct octree
    int x,y,z,index,ox,oy;
    int maxcoord=1<<ceillogbase2(MAX(MAX(vsize[0],vsize[1]),vsize[2]));
    int slicesize=vsize[0]*vsize[1];
    float* sliceData=new float[slicesize];
    octree* slice0=new octree[slicesize];
    octree* slice1=new octree[slicesize];
    octree* supervoxel=new octree();
    int numZeroVoxels=0;
    float val,min,max;
    ...
    //loop through the slices in pairs
    cout << "Reading " << vsize[2] << " slices"; cout.flush();
    for (z=0; z<vsize[2]; z+=2)
    {
        //read in one in-core slice of data
        cout << "."; cout.flush();
        in.read((char*)sliceData,slicesize*sizeof(float));
        index=0;
        for (y=0; y<vsize[1]; y++)
        {
            for (x=0; x<vsize[0]; x++)
            {
                //We treat 0.0 as a special value, so if necessary shift a little.
                if (ABS(sliceData[index])<SMALLNUM)
                {
                    sliceData[index]=(-SMALLNUM); //assume a 0 value meant slightly negative
                    numZeroVoxels++;
                }
                slice0[index].clear(sliceData[index]);
                checkSignDiff(x,y,z,index,sliceData,slice0,slice1,vsize,treeHeight);
                index++;
            }
        }
        ...
        if (z+1<vsize[2])
        {
            //do the same thing but swap the roles of the two in-core slices
            cout << ".";
            in.read((char*)sliceData,slicesize*sizeof(float));
            index=0;
            for (y=0; y<vsize[1]; y++)
            {
                for (x=0; x<vsize[0]; x++)
                {
                    ...
                    slice1[index].clear(sliceData[index]);
                    checkSignDiff(x,y,z+1,index,sliceData,slice1,slice0,vsize,treeHeight);
                    index++;
                }
            }
        }
        ...
        {
            //fill up a slice with BIGNUM to make an even number of slices
            for (index=0; index<slicesize; index++)
                slice1[index].clear(BIGNUM);
        }

        //loop over the 2x2 supervoxels
        for (y=0; y<vsize[1]; y+=2)
        {
            for (x=0; x<vsize[0]; x+=2)
            {
                supervoxel->clear(BIGNUM);
                min=BIGNUM; max=-BIGNUM;
                for (oy=0; oy<2; oy++) if (y+oy<vsize[1])
                    for (ox=0; ox<2; ox++) if (x+ox<vsize[0])
                    {
                        index=x+ox+(y+oy)*vsize[0];
                        val=slice0[index].getValue();
                        if (val<min) min=val;
                        if (val>max) max=val;
                        val=slice1[index].getValue();
                        if (val<min) min=val;
                        if (val>max) max=val;
                        if (slice0[index].getFlag(OTFL_CANTMERGE)) supervoxel->setFlag(OTFL_CANTMERGE);
                        if (slice1[index].getFlag(OTFL_CANTMERGE)) supervoxel->setFlag(OTFL_CANTMERGE);
                    }
                if (max<0.0) supervoxel->setValue(max);		//an inside node
                else if (min>0.0) supervoxel->setValue(min);	//an outside node
                else supervoxel->setValue(0);			//a mixed node
                //cout << '(' << supervoxel->getValue();
                //if (supervoxel->getFlag(OTFL_CANTMERGE)) cout << '*';
                //cout << ')';
                if (supervoxel->getFlag(OTFL_CANTMERGE))
                {
                    //create the children
                    //cout << '[';
                    supervoxel->initChildren();
                    levelOneVoxelsWithChildren++;
                    for (oy=0; oy<2; oy++) if (y+oy<vsize[1])
                        for (ox=0; ox<2; ox++) if (x+ox<vsize[0])
                        {
                            index=x+ox+(y+oy)*vsize[0];
                            //if (slice0[index].getFlag(OTFL_CANTMERGE))
                                supervoxel->setChild(childIndex(ox,oy,0),&slice0[index]);
                            //if (slice1[index].getFlag(OTFL_CANTMERGE))
                                supervoxel->setChild(childIndex(ox,oy,1),&slice1[index]);

                            //cout << '|';
                            //cout << slice0[index].getValue();
                            //if (slice0[index].getFlag(OTFL_CANTMERGE)) cout << '*';
                            //cout << '/';
                            //cout << slice1[index].getValue();
                            //if (slice1[index].getFlag(OTFL_CANTMERGE)) cout << '*';
                        }
                ...
                //Add supervoxel (x,y,z) to the tree and merge if applicable.
                this->addVoxel2(treeHeight,x,y,z,supervoxel,vsize);
            }
            //cout << '\n';
        }
    }
    ...
    //clear out data so that children aren't deleted
    for (index=0; index<slicesize; index++)
    {
        slice0[index].clear(0);
        slice1[index].clear(0);
    }
    supervoxel->clear(0);
    ...
    //cout << numVoxelsChildrenInited << " voxels with children created.\n";
    //cout << numVoxelsChildrenDeleted << " voxels with children deleted.\n";
    //cout << numVoxelsChildrenInited-numVoxelsChildrenDeleted+levelOneVoxelsWithChildren;
    //cout << " voxels with children.\n";
    //cout << (numVoxelsChildrenInited-numVoxelsChildrenDeleted+levelOneVoxelsWithChildren)*8+1;
    //cout << " total voxels.\n";
    //cout << countNodes() << " total voxels.\n";
}*/
	...
	for (z=0; z<vsize[2]; z++)
        for (y=0; y<vsize[1]; y++)
            for (x=0; x<vsize[0]; x++)
			{
				...
				if ((anc->getFlag(OTFL_UNKNOWN)) || (anc->getValue()>=0.0))
...
int octree::readVRI(char* filename,int* vsize,float isovalue)
{
    ...
    pa=0.21; pb=0.0; pc=1.0; pd=0.19; //"david-head-1mm-carve.vri"
    ...
    for (index=0; index<slicesize; index++)
        sliceData1[index]=isovalue-(float)occSlice[index].value;
    for (z=0; z<vsize[2]; z++)
    {
        ...
        if (z+1<vsize[2])
        {
            ...
            for (index=0; index<slicesize; index++)
                sliceData2[index]=isovalue-(float)occSlice[index].value;
        }
        else for (index=0; index<slicesize; index++) sliceData2[index]=BIGNUM;
        index=0;
        for (y=0; y<vsize[1]; y++)
        {
            for (x=0; x<vsize[0]; x++)
            {
                ...
                //We trim off a plane of junk data.
                if ((pa*x)/vsize[0]+(pb*y)/vsize[1]+(pc*z)/vsize[2]<pd)
                {
                    sliceData1[index]=BIGNUM;
                }

                //Init the new octree node
                slice1[index].clear(sliceData1[index]);
                if ((occSlice[index].totalWeight>0) &&
                    (ABS(sliceData1[index])<isovalue/2))
                    slice1[index].setFlag(OTFL_CANTMERGE);
                if (occSlice[index].totalWeight==0)
...
void octree::writeDF(ostream& out)
{
	...
	float val=value;
	if (getFlag(OTFL_UNKNOWN)) val=fabs(value);
