...
{
  Vector3D normal=cross(minus(b,a),minus(c,a));
  Vector3D crossA,crossB,crossC;
  crossA=cross(minus(c,b),minus(p,b));
  crossB=cross(minus(a,c),minus(p,c));
  crossC=cross(minus(b,a),minus(p,a));
  float dotA,dotB,dotC;
  dotA=dot(normal,crossA);
  dotB=dot(normal,crossB);
  dotC=dot(normal,crossC);
...
{
  Vector3D normal=normalized(cross(minus(b,a),minus(c,a)));
  return dot(normal,p)-dot(normal,a);
}
