  ...
  for (index=0; index<numVoxels; index++)
    {
      ...
      carvedRegions[component]->addToBoundary(pqitem(index,data[index]));
    }
  cout << "done.\n";
}
...
void volume::addNeighborsToQueue(pqitem voxel,int component)
{
	    ...
	    (x+ox>=0) && (x+ox<size[0]))
	{
		...
		pqitem neighbor(neighborIndex,voxel.value);
		//We set a flag so that this voxel is not delayed multiple times.
		neighbor.requeued=1;
  ...
  while (maxi>-1)
      ...
      if (carvedRegions[maxi]->getNumCarved() > 5000)
	{
	  ...
	  else
	  {
		  ...
		  //Add back to the queue with delayed priority, if it hasn't already been done.
		  if (!voxel.requeued) carvedRegions[maxi]->addDelayedToBoundary(voxel);
	      }
	  }
      }
