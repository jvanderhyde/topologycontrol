...
int volume::traceLowerCriticalAncestor(int index)
{
    int x,y,z,i,ox,oy,oz;
    int neighborIndex,minNeighbor=index;
    int anc=index;
    
    while (!voxelCritical(anc))
    {
	getVoxelLocFromIndex(anc,&x,&y,&z);
	for (i=0; i<numNeighborsVertex; i++)
	{
	...
	assert((voxelCritical(minNeighbor)) || (minNeighbor!=anc));
	anc=minNeighbor; //For next iteration initially minNeighbor==anc.
    }
    return anc;
}

void volume::calcCricitalParentsByTracing()
{
    int index;
	...
	while ((criticalParent[anc]==-1) && (!voxelCritical(anc)))
	{
	    getVoxelLocFromIndex(anc,&x,&y,&z);
	    for (i=0; i<numNeighborsVertex; i++)
	    {
	...
	if (criticalParent[anc]!=-1) criticalParent[index]=criticalParent[anc];
	else criticalParent[index]=anc;
    }
}
...
void volume::joinCriticalPoints(int index,int numNeighbors,int** neighborhood)
{
    int questionVoxel1=-1,questionVoxel2=-1,questionVoxel3=-1;
    int x,y,z,i,ox,oy,oz;
    int na;
	...
	if ((na != -1) && (na != index))
	{
	    if ((0) || (na==questionVoxel1) || (na==questionVoxel2) || (na==questionVoxel3))
	    {
...
void volume::buildJoinTreeForInsideOrder(int* order,ContourTree& jt)
{
	...
	if (voxelCritical(index,order))
	{
	    ...
	    joinCriticalPoints(index,numNeighborsVertex,neighborhoodVertex);
	}
	else
	{
...
void volume::labelFullJoinTree(int isovalue)
{
    fullJoinTree.calcJTIsovalueAncestors(isovalue,data);
}
