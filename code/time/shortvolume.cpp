...
{
  if (!order) order=getDefaultOrder();
  if (strictTopoCheck->topologyChange(getTopoType(index,order))) return 1;
  else return 0;
}
...
int main(int argc,char* argv[])
{
  ...
  v.sortVoxels();
  v.countCriticals(v.getDefaultOrder());
  ...
  v.saveOrder("carvingorder_strict.vo",v.getDefaultOrder());
