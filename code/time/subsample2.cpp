...
int main(int argc,char* argv[])
{
    if (argc<=2)
    {
	    cerr << "Uses middle half of the time slices.\n";
    ...
    cout << "Downsampling";
	int startSlice=vsize1[3]/4;
	for (t=0; t<startSlice; t++) fin.read((char*)vdata1,sliceSize1*sizeof(unsigned char));
    for (t=startSlice; t<startSlice+vsize1[3]/2; t++)
    {
	cout << "."; cout.flush();
	fin.read((char*)vdata1,sliceSize1*sizeof(unsigned char));
	index2=0;
