...
{
    if (x<0) return d(0,y,z);
    if (y<0) return d(x,0,z);
    if (z<0) return d(x,y,0);
    if (x>=size[0]) return d(size[0]-1,y,z);
    if (y>=size[1]) return d(x,size[1]-1,z);
    if (z>=size[2]) return d(x,y,size[2]-1);
    return d3(x,y,z);
...
MarchableVolume4D::MarchableVolume4D() : MarchableVolume::MarchableVolume()
{
    currentTimeSlice=0;
	numTimeSlices=-1;
}

MarchableVolume4D::~MarchableVolume4D()
{
}

void MarchableVolume4D::setCurrentTimeSlice(int s)
{
	currentTimeSlice=s;
}


FloatVolume4D::FloatVolume4D() : MarchableVolume4D::MarchableVolume4D()
{
	data=NULL;
}

FloatVolume4D::~FloatVolume4D()
{
	if (data) delete[] data;
}

void FloatVolume4D::copySlice(int z,float* slice)
{
	int offset=currentTimeSlice*numVoxelsInTimeSlice+z*sliceSize;
	for (int index=0; index<sliceSize; index++)
		slice[index]=data[offset+index];
}

void FloatVolume4D::storeSlice(int z,float* slice)
{
	int offset=currentTimeSlice*numVoxelsInTimeSlice+z*sliceSize;
	for (int index=0; index<sliceSize; index++)
		data[offset+index]=slice[index];
...
int FloatVolume4D::readFile(char* filename)
{
    ifstream fin(filename);
    if (!fin) return 1;
    fin.read((char*)size,sizeof(size));
	fin.read((char*)&numTimeSlices,sizeof(numTimeSlices));
	sliceSize=size[1]*size[0];
	numVoxels=size[2]*sliceSize;
	numVoxelsInTimeSlice=numVoxels;
	numTotalVoxels=numVoxelsInTimeSlice*numTimeSlices;
	data=new float[numTotalVoxels];
	
	float* slice=new float[sliceSize];
	int x,y,z,t,voxelIndex=0,index;
	for (t=0; t<numTimeSlices; t++)
	for (z=0; z<size[2]; z++)
	{
		index=0;
		fin.read((char*)slice,sliceSize*sizeof(float));
...
int FloatVolume4D::writeFile(char* filename)
{
	ofstream fout(filename);
	if (!fout) return 1;
	d(0,0,0);
	fout.write((char*)size,sizeof(size));
	fout.write((char*)&numTimeSlices,sizeof(numTimeSlices));
	fout.write((char*)data,numTotalVoxels*sizeof(float));
	fout.close();
	return 0;
}

int FloatVolume4D::isFloatingType()
{
    return 1;
}

ByteVolume4D::ByteVolume4D() : MarchableVolume4D::MarchableVolume4D()
{
  data=NULL;
}

ByteVolume4D::~ByteVolume4D()
{
  if (data) delete[] data;
}

void ByteVolume4D::copySlice(int z,float* slice)
{
  int offset=currentTimeSlice*numVoxelsInTimeSlice+z*sliceSize;
  for (int index=0; index<sliceSize; index++)
    slice[index]=(float)data[offset+index];
...
void ByteVolume4D::storeSlice(int z,float* slice)
{
  int offset=currentTimeSlice*numVoxelsInTimeSlice+z*sliceSize;
  for (int index=0; index<sliceSize; index++)
    data[offset+index]=(unsigned char)floor(slice[index]+0.5);
...
int ByteVolume4D::readFile(char* filename)
{
  ifstream fin(filename);
  if (!fin) return 1;
  fin.read((char*)size,sizeof(size));
  ...
  numVoxelsInTimeSlice=numVoxels;
  numTotalVoxels=numVoxelsInTimeSlice*numTimeSlices;
  data=new unsigned char[numTotalVoxels];
...
int ByteVolume4D::writeFile(char* filename)
{
  ofstream fout(filename);
  if (!fout) return 1;
  d(0,0,0);
  fout.write((char*)size,sizeof(size));
  fout.write((char*)&numTimeSlices,sizeof(numTimeSlices));
  fout.write((char*)data,numTotalVoxels*sizeof(unsigned char));
  fout.close();
  return 0;
}

int ByteVolume4D::isFloatingType()
{
    return 0;
}
