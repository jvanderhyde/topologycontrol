...
volume::volume(int sizex,int sizey,int sizez)
{
	...
	known.setbit(numVoxels-1);
	known.resetbit(numVoxels-1);
	carved.setbit(numVoxels-1);
	carved.resetbit(numVoxels-1);
	minvoxel.priority=0;
    animationOn=0;
    frameNumber=0;
	initialOrder=NULL;
...
int volume::getKnown(int index)
{
	return known.getbit(index);
}

void volume::setKnown(int index, int val)
{
	if (val) known.setbit(index);
	else known.resetbit(index);
}
...
void volume::calcDistances()
{
	...
	for (index=0; index<size[2]*size[1]*size[0]; index++)
	{
		...
		{
			//Start the queue with every voxel adjacent to isosurface 0
			getVoxelLocFromIndex(index,&x,&y,&z);
			onIsosurface=0;
...
void volume::carveFromInside(int lifeSpanThreshold,int calcLifeSpans)
{
	minqueue innerBoundary;
    pqitem top;
	int voxelIndex;
	...
	//cout << "Starting voxel: (" << minvoxel.x << ',' << minvoxel.y << ',' << minvoxel.z << ")... "; cout.flush();
	...
	while ((!innerBoundary.empty()) && (!infiniteLifeSpan)) //pq not empty
	{
		do
		{
			...
			voxelIndex=getVoxelIndex(top.x,top.y,top.z);
			queued.resetbit(voxelIndex);
		} while ((carved.getbit(voxelIndex)) && (!innerBoundary.empty()));
		if (carved.getbit(voxelIndex)) break;
		...
		else
		{
			...
			if ((calcLifeSpans) && (isInitiallyCritical(voxelIndex)))
			{
...
void volume::carveFromOutside(int lifeSpanThreshold,int calcLifeSpans)
{
	maxqueue outerBoundary;
    pqitem top;
	int voxelIndex;
	...
	while ((!outerBoundary.empty()) && (!infiniteLifeSpan)) //pq not empty
	{
			...
			((lifeSpanThreshold>-1) && (lifeSpans[voxelIndex]>=lifeSpanThreshold)))
		{
			...
			numCarvedPos++;
			//printVolume(cout); cout << '\n';
			if (topologyCheckOutside(top.x,top.y,top.z)) lifeSpans[voxelIndex]=size[0]*size[1]*size[2];
		}
		else
		{
			//If we find a critical voxel that can't be carved,
...
int volume::writeVFile(ostream& out)
{
	...
	for (z=0; z<size[2]; z++)
	{
		...
		for (y=0; y<size[1]; y++)
			for (x=0; x<size[0]; x++)
			{
				//We add a on fractional value to reflect the lexicographic sorting order.
				slice[index++]=data[voxelIndex]+voxelIndex/(float)numVoxels/2.0;
				voxelIndex++;
			}
