  ...
  : v(NULL), outside(),known(),carved(),side(whichside),isovalue(iso)
{
  ...
  {  
	  label=0;
	  if (mesh.getVert(mesh.getTri(i).a).label) label++;
	  if (mesh.getVert(mesh.getTri(i).b).label) label++;
	  if (mesh.getVert(mesh.getTri(i).c).label) label++;
...
void mcvolume::createTriangles(int i,int j,int k)
{
	...
	for ( int l=0;;l+=3 )
	{
		...
		if (tmap[index][l]==-1)
			break;
		mesh.addTri(Triangle(get_vid(i,j,k,tmap[index][l]),
							 get_vid(i,j,k,tmap[index][l+1]),
							 get_vid(i,j,k,tmap[index][l+2])));
		triangle_count++;
...
void mcvolume::createXVertices(int i,int j,int k)
{
	...
	if (get_intersection(d(i,j,k,&sus1),isovalue,d(i+1,j,k,&sus2),&res))
	{
		...
		if ((sus1) || (sus2))
			mesh.setVertLabel(point_count,1);
	    point_count++;
	}
}
