...
// result is a pointer to an array of lenx*leny*lenz nodes, which will be full of decendants of this node.
// All nodes in specified range are descendants of this node.
void octree::getRange(int level,int startx,int starty,int startz,int lenx,int leny,int lenz,octree** result,octree** space)
{
	int i,rx,ry,rz;
	//Do something here to fill in the blanks depending on what we need this function for
	for (i=0; i<lenx*leny*lenz; i++) result[i]=NULL;
	if (this==NULL) return;
	if (children != NULL)
	{
		int x,y,z;
		...
		children[c]->getRange(level-1,startx,starty,startz,lenx-lenx2,leny-leny2,lenz-lenz2,result+num,space+num);
		for (rz=0; rz<lenz-lenz2; rz++)
			for (ry=0; ry<leny-leny2; ry++)
				for (rx=0; rx<lenx-lenx2; rx++)
					space[num+rx+(lenx-lenx2)*(ry+(leny-leny2)*(rz))]=result[num++];
		if (lenx2>0)
		{
			children[childIndex(level,startx2,starty,startz)]->
				getRange(level-1,startx2,starty,startz,lenx2,leny-leny2,lenz-lenz2,result+num,space+num);
			for (rz=0; rz<lenz-lenz2; rz++)
				for (ry=0; ry<leny-leny2; ry++)
					for (rx=lenx-lenx2; rx<lenx; rx++)
						space[num+rx+(lenx2)*(ry+(leny-leny2)*(rz))]=result[num++];
		}
		if (leny2>0)
		{
			children[childIndex(level,startx,starty2,startz)]->
				getRange(level-1,startx,starty2,startz,lenx-lenx2,leny2,lenz-lenz2,result+num,space+num);
			for (rz=0; rz<lenz-lenz2; rz++)
				for (ry=leny-leny2; ry<leny; ry++)
					for (rx=0; rx<lenx-lenx2; rx++)
						space[num+rx+(lenx-lenx2)*(ry+(leny2)*(rz))]=result[num++];
		}
		if (lenz2>0)
		{
			children[childIndex(level,startx,starty,startz2)]->
				getRange(level-1,startx,starty,startz2,lenx-lenx2,leny-leny2,lenz2,result+num,space+num);
			for (rz=lenz-lenz2; rz<lenz; rz++)
				for (ry=0; ry<leny-leny2; ry++)
					for (rx=0; rx<lenx-lenx2; rx++)
						space[num+rx+(lenx-lenx2)*(ry+(leny-leny2)*(rz))]=result[num++];
		}
		if ((lenx2>0) && (leny2>0))
		{
			...
			getRange(level-1,startx2,starty2,startz,lenx2,leny2,lenz-lenz2,result+num,space+num);
			for (rz=0; rz<lenz-lenz2; rz++)
				for (ry=leny-leny2; ry<leny; ry++)
					for (rx=lenx-lenx2; rx<lenx; rx++)
						space[num+rx+(lenx2)*(ry+(leny2)*(rz))]=result[num++];
		}
		if ((leny2>0) && (lenz2>0))
		{
			children[childIndex(level,startx,starty2,startz2)]->
			getRange(level-1,startx,starty2,startz2,lenx-lenx2,leny2,lenz2,result+num,space+num);
			for (rz=lenz-lenz2; rz<lenz; rz++)
				for (ry=leny-leny2; ry<leny; ry++)
					for (rx=0; rx<lenx-lenx2; rx++)
						space[num+rx+(lenx-lenx2)*(ry+(leny2)*(rz))]=result[num++];
		...
		if ((lenz2>0) && (lenx2>0))
		{
			...
			getRange(level-1,startx2,starty,startz2,lenx2,leny-leny2,lenz2,result+num,space+num);
			for (rz=lenz-lenz2; rz<lenz; rz++)
				for (ry=0; ry<leny-leny2; ry++)
					for (rx=lenx-lenx2; rx<lenx; rx++)
						space[num+rx+(lenx2)*(ry+(leny-leny2)*(rz))]=result[num++];
		...
		if ((lenx2>0) && (leny2>0) && (lenz2>0))
		{
			...
			getRange(level-1,startx2,starty2,startz2,lenx2,leny2,lenz2,result+num,space+num);
			for (rz=lenz-lenz2; rz<lenz; rz++)
				for (ry=leny-leny2; ry<leny; ry++)
					for (rx=lenx-lenx2; rx<lenx; rx++)
						space[num+rx+(lenx2)*(ry+(leny2)*(rz))]=result[num++];
		...
		for (i=0; i<lenx*leny*lenz; i++) result[i]=space[i];
	}
}
