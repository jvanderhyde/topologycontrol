...
//Returns true if the voxel at the given coordinates has a neighbor of opposite sign
            ...
            for (x=0; x<size[0]; x+=stepSize)
            {
                ...
                if ((supervoxel != NULL) && (!supervoxel->getFlag(OTFL_CARVED)))
                {
                    ...
                    else for (c=0; c<8; c++)
                    {
						...
						if (voxelOnIsosurface(level-1,x+octree::childX(c),y+octree::childY(c),z+octree::childZ(c),
											  child))
...
int volume::topologyCheckInside(int level,int x,int y,int z)
{
    ...
    topoType ^= ((1<<26)-1); //sense of carved is opposite compared to carving check from outside
    return !!(topoinfo[topoType >> 3] & (1<<(topoType & 0x07)));
}
#endif
...
void volume::carveVoxelOutside(octree* voxel,int level,int x,int y,int z,octree** neighborsO,pqitem* neighborsQ)
{
    ...
    //check the neighbors
    num=getNeighbors26(level,x,y,z,neighborsO,neighborsQ); //why is this 26??
    for (n=0; n<num; n++)
    {
        if ((!neighborsO[n]->getFlag(OTFL_CARVED)) &&
