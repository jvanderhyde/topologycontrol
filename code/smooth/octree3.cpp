...
void octree::getRange(int level,int downToLevel,int startx,int starty,int startz,int lenx,int leny,int lenz,
		      octree** result,octree** space,int deepest,octree* dataroot)
{
  //if (dataroot) dataroot->print3();
  if (level>downToLevel)
      ...
      if (children != NULL)
	{
	  ...
	  int c=childIndex(level,startx,starty,startz),num=0;
	  ...
	  if (children[c] != NULL)
	    {
	      //recurse to child
	      children[c]->getRange(level-1,downToLevel,startx,starty,startz,lenx1,leny1,lenz1,result+num,space+num,deepest,dataroot);
	      //reorder
	      for (rz=0; rz<lenz1; rz++) for (ry=0; ry<leny1; ry++) for (rx=0; rx<lenx1; rx++)
		space[rx+lenx*(ry+leny*rz)]=result[num++];
	    }
	  else 
	    {
	      //no child, so just fill in default value
	      for (rz=0; rz<lenz1; rz++) for (ry=0; ry<leny1; ry++) for (rx=0; rx<lenx1; rx++)
		space[rx+lenx*(ry+leny*rz)]=defaultResult;
	      num+=lenx1*leny1*lenz1;
	    }
	  ...
	  if (lenx2>0)
	    {
	      ...
	      if (children[c] != NULL)
		{
		  children[c]->getRange(level-1,downToLevel,startx2,starty,startz,lenx2,leny1,lenz1,result+num,space+num,deepest,dataroot);
	  ...
	  if (leny2>0)
	    {
	      ...
	      if (children[c] != NULL)
		{
		  children[c]->getRange(level-1,downToLevel,startx,starty2,startz,lenx1,leny2,lenz1,result+num,space+num,deepest,dataroot);
	  ...
	  if (lenz2>0)
	    {
	      ...
	      if (children[c] != NULL)
		{
		  children[c]->getRange(level-1,downToLevel,startx,starty,startz2,lenx1,leny1,lenz2,result+num,space+num,deepest,dataroot);
	  ...
	  if ((lenx2>0) && (leny2>0))
	    {
	      ...
	      if (children[c] != NULL)
		{
		  children[c]->getRange(level-1,downToLevel,startx2,starty2,startz,lenx2,leny2,lenz1,result+num,space+num,deepest,dataroot);
	  ...
	  if ((leny2>0) && (lenz2>0))
	    {
	      ...
	      if (children[c] != NULL)
		{
		  children[c]->getRange(level-1,downToLevel,startx,starty2,startz2,lenx1,leny2,lenz2,result+num,space+num,deepest,dataroot);
	  ...
	  if ((lenz2>0) && (lenx2>0))
	    {
	      ...
	      if (children[c] != NULL)
		{
		  children[c]->getRange(level-1,downToLevel,startx2,starty,startz2,lenx2,leny1,lenz2,result+num,space+num,deepest,dataroot);
	  ...
	  if ((lenx2>0) && (leny2>0) && (lenz2>0))
	    {
	      ...
	      if (children[c] != NULL)
		{
		  children[c]->getRange(level-1,downToLevel,startx2,starty2,startz2,lenx2,leny2,lenz2,result+num,space+num,deepest,dataroot);
	  ...
	  //copy the corrected order into result
	  assert(num==lenx*leny*lenz);
	  for (int i=0; i<num; i++) result[i]=space[i];
	}
      else
	{
	  //I don't have descendants on the requested level.
	  for (int i=0; i<lenx*leny*lenz; i++) result[i]=defaultResult;
	}
    }
  else
    {
...
void octree::print2(int depth)
{
    ...
    if (children != NULL)
        for (int c=0; c<8; c++)
	{
	    if (depth==maxDepth-2) if (c>0) for (int i=0; i<depth+1; i++) cout << " ";
            if (children[c] != NULL)
                children[c]->print2(depth+1);
	}
    if (depth==maxDepth-1) cout << '\n';
}

void octree::print3(int depth)
{
    cout << this;
    if (children != NULL)
    {
