...
enum
{
    ...
    OTFL_DONTCARVNEG=64,//avoid this voxel when carving from inside
    OTFL_DONTCARVPOS=128//avoid this voxel when carving from outside
};
...
public:
    ...
    void getRange(int level,int downToLevel,int startx,int starty,int startz,int lenx,int leny,int lenz,octree** result,int rsx,int rsy,int rsz,int rlx,int rly,int rlz,int deepest);
    ...
    void setUnknownIfNotCantMergeAtLevel(int level,int downToLevel);
    void setUnknownIfNotCantMergeRecursively();
    ...
    void readData(istream& in,const int vsize[3],int treeHeight,float isovalue=0.0);
    ...
    int readFile(char* filename,int* vsize,float isovalue=0.0);
