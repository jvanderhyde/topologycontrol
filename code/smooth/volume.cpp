...
pqitem::pqitem ( MEASURE p, int xx, int yy, int zz ) : priority(p),x(xx),y(yy),z(zz) {}

pqitem::operator MEASURE()
{
    return priority;
}
...
MEASURE volume::getVoxel(int x,int y,int z)
{
	return data[getVoxelIndex(x,y,z)];
}

void volume::setVoxel(int x,int y,int z,MEASURE val)
{
...
int volume::openFeatureInside(intvector& forLater,maxqueue& boundary,int* neighbors,pqitem* neighborsQ)
{
	...
	if (maxindex>-1)
	{
		patchCentersInside.push_back(maxindex); //save it for future checks
		cout << "Inside patch voxel: " << top << "\t";
...
int volume::openFeatureOutside(intvector& forLater,maxqueue& boundary,int* neighbors,pqitem* neighborsQ)
{
	...
	if (maxindex>-1)
	{
		patchCentersOutside.push_back(maxindex); //save it for future checks
		cout << "Outside patch voxel: " << top << "\n";
...
void volume::calcDistances()
{
	...
	int onIsosurface;
    int top;
	MEASURE curdistance=0;
	int layerSize=0,newLayerSize=0;
	MEASURE signedDistance;
...
void volume::carveInside(int numFeaturesToOpen)
{
    cout << "Carving from inside... "; cout.flush();
	...
	intvector innerForLater;
	int anyUncarvedVoxelsLeft=1;
	
	carved.clearall();
	while (!patchCentersInside.empty()) patchCentersInside.pop_back();
	
	//carve minimum voxel to start inner boundary
	//pqitem startvoxel(-1,192,140,200);
	pqitem startvoxel(-1,size[0]/2,size[1]/2,size[2]/2-10);
	...
	while ((numFeatures<numFeaturesToOpen) && (anyUncarvedVoxelsLeft))
	{
		...
		while (!innerBoundary.empty()) //pq not empty
		{
			//pop supervoxel from top of priority queue
		...
		if (numFeatures<numFeaturesToOpen)
		{
			...
			if (openedInside==0) anyUncarvedVoxelsLeft=0;
		}
		
	}
	
    cout << numCarvedNeg << " carved from inside.\n";
	if (numFeaturesToOpen>0) cout << numFeaturesOpened << " topology-altering operations performed.\n";
	renderVolume(1);
}

void volume::carveOutside(int numFeaturesToOpen)
{
    cout << "Carving from outside... \n"; cout.flush();
	...
	int numFeatures=-1; //set up to be corrected when entering the while loop
	int numFeaturesOpened=0;
	intvector outerForLater;
	int anyUncarvedVoxelsLeft=1;
	
	carved.clearall();
	while (!patchCentersOutside.empty()) patchCentersOutside.pop_back();
	...
	while ((numFeatures<numFeaturesToOpen) && (anyUncarvedVoxelsLeft))
	{
		...
		//carve without changing topology
		while (!outerBoundary.empty()) //pq not empty
		{
			//pop supervoxel from top of priority queue
		...
		if (numFeatures<numFeaturesToOpen)
		{
			...
			if (openedOutside==0) anyUncarvedVoxelsLeft=0;
		}
...
int volume::guessNumExtraFeatures(int numFeaturesToOpen)
{
	int r;
	if ((patchCentersInside.size()<numFeaturesToOpen) || (patchCentersOutside.size()<numFeaturesToOpen))
		r=0;
	else
	{
		r=0;
		while ((patchCentersOutside.size()>r+numFeaturesToOpen) && 
			   (ABS(data[patchCentersOutside[r+numFeaturesToOpen]]) >
				ABS(data[patchCentersInside[numFeaturesToOpen]])))
			r++;
	}
	cout << "Heuristic guesses " << r << " extra features.\n";
	return r;
}
