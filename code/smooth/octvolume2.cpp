...
{
  int stepSize=1<<level;
  int startx=x-1,starty=y-1,startz=z-1,endx=x+1,endy=y+1,endz=z+1;
  if (startx<0) startx=0;
  if (starty<0) starty=0;
  if (startz<0) startz=0;
  if (endx>size[0]-1) endx=size[0]-1;
  if (endy>size[1]-1) endy=size[1]-1;
  if (endz>size[2]-1) endz=size[2]-1;
  int n=(endx-startx+1)*(endy-starty+1)*(endz-startz+1);
  dataroot->getRange(treeHeight,startx,starty,startz,endx-startx+1,endy-starty+1,endz-startz+1,octree** neighborsO);
  return n;
}
