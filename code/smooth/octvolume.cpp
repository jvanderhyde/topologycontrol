...
int volume::readTopoinfoFiles()
{
  ...
  //ifstream finp("topoinfo_vertex_all");
  if (!finp)
    {
  ...
  //ifstream fint("topoinfo_vertex_all");
  if (!fint)
    {
...
//Sets all voxels so that only voxels that are on the isosurface and have CANTMERGE set are known.
void volume::resetUnknowns(int level)
{
  int x,y,z;
  ...
  cout << " Marking known voxels..."; cout.flush();
  for (z=0; z<size[2]; z+=stepSize)
    for (y=0; y<size[1]; y+=stepSize)
      for (x=0; x<size[0]; x+=stepSize)
	{
	  voxel=getDeepestVoxel(x,y,z);
	  if ((voxel->getFlag(OTFL_CANTMERGE)) && (voxelOnIsosurface(level,x,y,z,voxel)))
	    {
	      //This should only be highest-resolution voxels because every supervoxel with CANT_MERGE set has children.
	      voxel->setFlag(OTFL_VALID);
	      averageVal+=fabs(voxel->getValue());
	      numKnown++;
	    }
	  else
	    voxel->clearFlag(OTFL_VALID);
	}
  for (z=0; z<size[2]; z+=stepSize)
    for (y=0; y<size[1]; y+=stepSize)
      for (x=0; x<size[0]; x+=stepSize)
	if (!(voxel=getDeepestVoxel(x,y,z))->getFlag(OTFL_VALID))
	    voxel->setFlag(OTFL_UNKNOWN);
  cout << "average value is " << averageVal/numKnown << ".\n";
}
...
int volume::voxelOnIsosurface(int level,int x,int y,int z,octree* givenVoxel)
{
  ...
  /*octree* n;
    int ox,oy,oz;*/
  octree* neighborsO[27];
  int numNeighbors=getNeighbors27(level,x,y,z,neighborsO,NULL,1);
  for (int i=0; i<numNeighbors; i++)
    if ((!neighborsO[i]->getFlag(OTFL_UNKNOWN)) && (SGN(neighborsO[i]->getValue()) != s)) return 1;
...
// may border on an unknown but carved from inside voxel.
//It's tempting to do this at octree construction time instead.
// Since supervoxels with children can't be carved, we could just save all supervoxels
// two away from the isosurface.
// But we don't know where the new nodes have to be refined until the carving procedure.
void volume::uncarveLayer(int level)
{
  ...
  int stepSize=1<<level;
  octree* voxel;
  pqitem neighborsQ[27];
  octree* neighborsO[27];
  int s;
  int uncarveThisVoxel,topoChange;
  pqvector toUncarve;
  pqitem top;
  int numUncarvedNeg=0,numUncarvedPos=0;
  int numUncarvedThisTime=-1;
  int stricterTopologyCheck;
		...
		if (voxel->getFlag(OTFL_CARVED))
		  {
		    ...
		    num=getNeighbors27(level,x,y,z,neighborsO,neighborsQ);
		    for (n=0; n<num; n++) if (neighborsO[n] != NULL)
		      {
  ...
  //Regarding the strictness of the topology check:
  // Techincally what we need is the patch check for outside and the thread check for inside.
  // Indeed, there is an order to uncarving that will do everything correctly.
  // If this ordering is not obeyed, handles can be created. These handles do not cause
  // a problem because they are immediatedly fixed by the next carving stage. However,
  // they can make the uncarving process unable to proceed and therefore defeat its purpose.
  // To get closer to the right ordering, we do a strict topology check to prevent the creation
  // of handles, because any handles that might be created can be carved out in a topology-preserving
  // order. My earlier conclusion that the topology checks simply needed to be reversed was not
  // correct because this causes components to be split apart, and then the topology check during
  // carving prevents them from coming back together.

  for (stricterTopologyCheck=0; stricterTopologyCheck<2; stricterTopologyCheck++)
    {
      numUncarvedThisTime=-1;
	  ...
	  for (pqvector::iterator i=toUncarve.begin(); i!=toUncarve.end(); i++)
	    {
	      ...
	      if (top.priority>0.0)
		{
		  if (stricterTopologyCheck==0)
		    topoChange=((topologyCheckOutside(level,top.x,top.y,top.z,topoinfoThread)) ||
				(topologyCheckOutside(level,top.x,top.y,top.z,topoinfoPatch)));
		  else topoChange=topologyCheckOutside(level,top.x,top.y,top.z,topoinfoPatch);
		}
	      else
		{
		  if (stricterTopologyCheck==0)
		    topoChange=((topologyCheckInside(level,top.x,top.y,top.z,topoinfoThread)) ||
				(topologyCheckInside(level,top.x,top.y,top.z,topoinfoPatch)));
		  else topoChange=topologyCheckInside(level,top.x,top.y,top.z,topoinfoThread);
		}
  ...
  if (numUncarvedNeg>numCarvedNegative) alreadyCarvedNegative=0;

  cout << ' ' << numUncarvedNeg << " from inside, " << numUncarvedPos << " from outside.\n";
}
...
int volume::getNeighbors27(int level,int x,int y,int z,octree** neighborsO,pqitem* neighborsQ,int deepest)
{
  int stepSize=1<<level;
  ...
  int n=lenx*leny*lenz;
  dataroot->getRange(treeHeight,level,startx,starty,startz,lenx,leny,lenz,neighborsO,startx,starty,startz,lenx,leny,lenz,deepest);
  if (neighborsQ)
    {
      int ix,iy,iz,index=0;
      for (iz=startz; iz<=endz; iz+=stepSize) for (iy=starty; iy<=endy; iy+=stepSize) for (ix=startx; ix<=endx; ix+=stepSize)
	{
	  if (neighborsO[index] != NULL) neighborsQ[index]=pqitem(neighborsO[index]->getValue(),ix,iy,iz);
	  index++;
	}
    }
  return n;
...
//Returns the number of non-null, in-range neighbors around the given voxel.
// The neighbors are stored in both neighborsQ and neighborsO.
int volume::getNeighbors26(int level,int x,int y,int z,octree** neighborsO,pqitem* neighborsQ,int deepest)
{
    int n=0;
    int stepSize=1<<level;
    int ox,oy,oz;
                ...
                if ((y+oy>=0) && (y+oy<size[1]))
                    for (ox=-stepSize; ox<=stepSize; ox+=stepSize)
                        if ((x+ox>=0) && (x+ox<size[0]))
                            if ((oz!=0) || (oy!=0) || (ox!=0))
                            {
                                if (deepest) neighborsO[n]=getDeepestVoxel(x+ox,y+oy,z+oz);
				else neighborsO[n]=getVoxel(level,x+ox,y+oy,z+oz);
                                if ((neighborsQ) && (neighborsO[n] != NULL)) neighborsQ[n]=pqitem(neighborsO[n]->getValue(),x+ox,y+oy,z+oz);
				n++;
                            }
    return n;
}
...
// The neighbors are stored in both neighborsQ and neighborsO.
/*int volume::getNeighbors124(int level,int x,int y,int z,octree** neighborsO,pqitem* neighborsQ)
{
  int stepSize=1<<level;
  int startx=x-2*stepSize,starty=y-2*stepSize,startz=z-2*stepSize,endx=x+2*stepSize,endy=y+2*stepSize,endz=z+2*stepSize;
  while (startx<0) startx+=stepSize;
  while (starty<0) starty+=stepSize;
  while (startz<0) startz+=stepSize;
  while (endx>size[0]-1) endx-=stepSize;
  while (endy>size[0]-1) endy-=stepSize;
  while (endz>size[0]-1) endz-=stepSize;
  int lenx=(endx-startx+stepSize)/stepSize,leny=(endy-starty+stepSize)/stepSize,lenz=(endz-startz+stepSize)/stepSize;
...
int volume::getNeighbors124(int level,int x,int y,int z,octree** neighborsO,pqitem* neighborsQ)
{
    ...
    int stepSize=1<<level;
    int ox,oy,oz;
    for (oz=-2*stepSize; oz<=2*stepSize; oz+=stepSize) if ((z+oz>=0) && (z+oz<size[2]))
      for (oy=-2*stepSize; oy<=2*stepSize; oy+=stepSize) if ((y+oy>=0) && (y+oy<size[1]))
	for (ox=-2*stepSize; ox<=2*stepSize; ox+=stepSize) if ((x+ox>=0) && (x+ox<size[0]))
	  if ((oz!=0) || (oy!=0) || (ox!=0))
	    {
...
//Now uses a poor man's Euclidean distance.
		...
		for (n=0; n<num; n++) if (neighborsO[n] != NULL)
		  {
			...
			(!neighborsO[n]->getFlag(OTFL_VALID)))
		      {
			val=neighborsO[n]->getValue();
			neighborsQ[n].priority=ABS(voxel->getValue())+dist(x,y,z,neighborsQ[n].x,neighborsQ[n].y,neighborsQ[n].z);
			if ((!neighborsO[n]->getFlag(OTFL_QUEUED)) ||
			    (neighborsQ[n].priority<ABS(val)))
			{
			  pqabs.push(neighborsQ[n]);
			  neighborsO[n]->setFlag(OTFL_QUEUED);
			  neighborsO[n]->setValue(SGN(val)*neighborsQ[n].priority);
...
int volume::topologyCheckOutside(int level,int x,int y,int z,unsigned char* topoinfo)
{
    return 0;
}

int volume::topologyCheckInside(int level,int x,int y,int z,unsigned char* topoinfo)
{
    return 0;
}
...
int volume::topologyCheckOutside(int level,int x,int y,int z,unsigned char* topoinfo)
{
      ...
      for (ox=-stepSize; ox<=stepSize; ox+=stepSize)
	{
	  if ((oz!=0) || (oy!=0) || (ox!=0))
		  ...
		  (x+ox>=0) && (x+ox<size[0]))
		{
		  ...
		  if ((!neighborsO[n]->getFlag(OTFL_CARVED)) || (neighborsO[n]->getValue()<0))
		    topoType |= (1<<bit);
		  n++;
		}
	      bit++;
  ...
  if (useStrictTopologyCheck)
    {
      return ((!!(topoinfoPatch[topoType >> 3] & (1<<(topoType & 0x07)))) || 
	      (!!(topoinfoThread[topoType >> 3] & (1<<(topoType & 0x07)))));
    }
  else
    {
...
/*int volume::topologyCheckOutside(int level,int x,int y,int z)
{
  ...
  //If we assume the object has only one component and has genus 0,
  // we're only concerned with patching over through-holes.
  return !!(topoinfoPatch[topoType >> 3] & (1<<(topoType & 0x07)));
}
...
{
  //if ((87<=x) && (x<87+6) && (87<=y) && (y<87+4) && (37<=z) && (z<37+4)) printRegion(level,cout,87,87,37,6,4,4);
    ...
    if (printCarveDetails)
      {
	cout << ' ' << '(' << x << ',' << y << ',' << z << ')' << ':' << voxel->getValue();
	cout << ' ' << ((topologyCheckOutside(level,x,y,z,topoinfoPatch))?"topo change":"no change");
	cout << '\n';
      }
    //check the neighbors
    num=getNeighbors27(level,x,y,z,neighborsO,neighborsQ);
    for (n=0; n<num; n++) if (neighborsO[n] != NULL)
    {
	    ...
	    (!neighborsO[n]->getFlag(OTFL_DONTCARVPOS)))
        {
	    ...
	    //After we push, we set the DONTCARVENEG flag so that carving from inside can't leak into patches.
	    neighborsO[n]->setFlag(OTFL_DONTCARVNEG);
        }
    }
    //cout << " added to outer boundary.\n";
...
void volume::carveVoxelInside(octree* voxel,int level,int x,int y,int z,octree** neighborsO,pqitem* neighborsQ)
{
    ...
    for (n=0; n<num; n++) if (neighborsO[n] != NULL)
    {
        if ((!neighborsO[n]->getFlag(OTFL_CARVED)) &&
            (!neighborsO[n]->getFlag(OTFL_BOUNDARY))  &&
            ((neighborsO[n]->getFlag(OTFL_UNKNOWN)) || (neighborsO[n]->getValue() < 0)) &&
	    (!neighborsO[n]->getFlag(OTFL_DONTCARVNEG)))
        {
            //add this neighbor to priority queue
            neighborsO[n]->setFlag(OTFL_BOUNDARY);
            neighborsQ[n].priority=ABS(neighborsQ[n].priority);
	    ...
	    //After we push, we set the DONTCARVEPOS flag so that carving from outside can't leak into patches.
	    neighborsO[n]->setFlag(OTFL_DONTCARVPOS);
        }
    }
    //cout << " added to inner boundary.\n";
    //if (animationOn) if (fixStyle<=0) renderVolume(level);
...
void volume::carveSimultaneousLowRes(int level)
{
    ...
    while ((!outerBoundary->empty()) || (!innerBoundary->empty())) //pq not empty
    {
        ...
        if (useNegative)
        {
            if ((!topologyCheckInside(level,top.x,top.y,top.z,topoinfoThread)) &&
        ...
        {
            if ((!topologyCheckOutside(level,top.x,top.y,top.z,topoinfoPatch)) &&
                (!voxel->getFlag(OTFL_CANTMERGE)))
            {
                //If it doesn't, carve this voxel.
    ...
    numCarvedNegative=numCarvedNeg;
...
void volume::fixVolumeOutside(int level)
{
    ...
    for (z=0; z<size[2]; z+=stepSize)
        for (y=0; y<size[1]; y+=stepSize)
		    ...
		    else voxel->setFlag(OTFL_CARVED); //set negative voxels to carved for drawing purposes
		...
		else if (voxel != NULL)
		  {
		    ...
		    //Voxels on isosurface that can't be merged have valid distance.
		    if ((voxel->getFlag(OTFL_CANTMERGE)) && (voxelOnIsosurface(level,x,y,z,voxel))) voxel->setFlag(OTFL_VALID);
		  }
            }
		...
		if (voxel != NULL)
		  {
			...
			else voxel->setFlag(OTFL_CARVED); //set positive voxels to carved for drawing purposes
...
void volume::fixTopology()
{
    ...
    //printRegion(level,cout,87,87,37,6,4,4);
    for (; level>0; level--)
    {
        cout << "level " << level << ":\n";
        calcDistances(level);
...
void volume::renderVolume(int level)
{
  ...
  //if (frameNumber%100!=1) return;
  //if (frameNumber<30160) return;
  //if (frameNumber>32700) return;
  //loose:  8001, 18001, 28771, 28781, 30021, 30131, 30161, 32701
  //strict: 8001, 18001, 28771, 28781, 30021, 30301, 31101, 32701
  if ((frameNumber!=8001) && (frameNumber!=18001) && (frameNumber!=28771) && (frameNumber!=28781) && 
      (frameNumber!=30021) && (frameNumber!=30131) && 
      (frameNumber!=30161) && (frameNumber!=30301) && (frameNumber!=31101) && (frameNumber!=32701)) return;
  ...
  char filename[20];
  sprintf(filename,"frames/frame_%.5d.v",frameNumber);
  ...
  if (fout)
    {
	  ...
	  for (y=0; y<coords[1]; y++)
		...
		if (renderStyle==3) //carving progress from side indicated by fixStyle
		  {
		    if (fixStyle != 0)
		      {
			if ((SGN(voxel->getValue())==fixStyle) && (voxel->getFlag(OTFL_CARVED)))
			  vdata[index]=fixStyle*1.0*ABS(voxel->getValue());
			else
			  vdata[index]=-fixStyle*1.0*ABS(voxel->getValue());
		      }
		    else
		...
		else if (renderStyle==4) //just uncarved voxels on side indicated by fixStyle
		  {
		    if (fixStyle != 0)
		      {
			if ((SGN(voxel->getValue())==fixStyle) && (!voxel->getFlag(OTFL_CARVED)))
		...
		else if (renderStyle==5) //unknown voxels
		{
		    if (voxel->getFlag(OTFL_UNKNOWN))
			vdata[index]=-1.0;
		    else
			vdata[index]=1.0;
		    if ((x*stepsize>=size[0]) || (y*stepsize>=size[1]) ||(z*stepsize>=size[2]))
		...
		else if (renderStyle==2) //boundary
		  {
...
void volume::printRegion(int level,ostream& out,int startx,int starty,int startz,int lenx,int leny,int lenz)
{
    int x,y,z;
    int stepSize=1<<level;
    octree* voxel;
    float val;
    for (z=0; z<lenz; z+=stepSize)
    {
        for (y=0; y<leny; y+=stepSize)
        {
            for (x=0; x<lenx; x+=stepSize)
            {
                voxel=getVoxel(level,x+startx,y+starty,z+startz);
                if (voxel != NULL)
                {
                    if (voxel->getFlag(OTFL_CARVED)) out << 'x';
                    else if (voxel->getFlag(OTFL_BOUNDARY)) out << '|';
		    ...
		    val=voxel->getValue();
		    if (voxel->getFlag(OTFL_UNKNOWN)) out << '?';
		    else if (val<0.0) out << '-';
		    else if (val>0.0) out << '+';
		    else out << ' ';
                    out << fabs(trunc(val));
...
int main(int argc,char* argv[])
{
    if (argc<=3)
    {
        cerr << "Usage: " << argv[0] << " <input .v file> <output .v file> <number of features> [inside|outside [<starting level>]]\n";
        return 1;
    }
    ...
    if (argc>5) startingLevel=atoi(argv[5]);

    int result;

    volume v(atoi(argv[3]),inside,startingLevel);
