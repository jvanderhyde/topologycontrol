...
int main(int argc,char* argv[])
{
	...
	if (inside==0)
	{
		...
		/*v.carveSimultaneously(v.getSize()[0]*v.getSize()[1]*v.getSize()[2]);
		v.carveOutside(numFeatures+v.guessNumExtraFeatures(numFeatures));
		v.fixVolumeTopology(1);
		v.carveInside(numFeatures);
		v.fixVolumeTopology(-1);*/
	}
	else
	{
		v.carveSimultaneously(numFeatures);
		v.fixVolumeTopology(inside);
	}
    endtime = clock();
    cout << "Running time: " << (endtime-starttime)/(double)CLOCKS_PER_SEC << " sec\n";
